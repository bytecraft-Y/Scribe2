...
with col_controls:
    with st.container(height=720, border=True):
        ...
        if input_source == "📁 Local Upload":
            ...
            media_url = st.text_input("🔗 Paste Web URL (YouTube, Vimeo, Podcast)", placeholder="https://...")
            if media_url:
                if st.button("⬇️ Fetch from Internet", use_container_width=True):
        ...
        if input_source == "🌐 Internet Link" and "fetched_web_file" in st.session_state:
            ...
            if st.button("🗑️ Clear Web Media", use_container_width=True):
...
with col_output:
    with st.container(height=720, border=True):
        if not st.session_state.segments_data:
            ...
            tab_transcript, tab_ai = st.tabs(["📄 Raw Transcript", "✨ AI Insights"])
            ...
            with tab_ai:
                st.markdown("### 🧠 Advanced Knowledge Extraction Engine")
                
                api_key_input = st.text_input(
                    "🔑 Enter OpenAI API Key for Generative Intelligence (Optional)", 
                    type="password",
                    placeholder="sk-...",
                    help="Leave blank to use the local offline TF-IDF engine."
                )
                
                st.write("---")
                
                if st.button("✨ Execute Strategic NLP Pipeline", type="primary"):
                    with st.spinner("Processing enterprise transcript vectors..."):
                        ...
                        if api_key_input.strip().startswith("sk-"):
                            try:
                                ...
                                system_instruction = "You are an elite corporate systems analyst. Analyze the meeting transcript and compile a definitive, enterprise-grade analysis brief formatted in Markdown."
                                user_prompt = f"Analyze this transcript and output:\n1. Top 5 contextual tags.\n2. An Executive Summary.\n3. Extracted Action Items.\n4. Key Decisions & Core Insights.\n\nTranscript:\n{raw_transcript}"
                                ...
                                st.toast("Generative AI Extraction Successful!", icon="🚀")
                        ...
                        # CHOICE B: LOCAL TF-IDF ENGINE (Fallback)
                            ...
                            st.info("ℹ️ No active API key detected. Deploying local TF-IDF statistical engine...")
                            ...
                            if not sentences:
                                ...
                                output = f"**🏷️ Auto-Generated Tags (Local TF-IDF):** {', '.join([k.capitalize() for k in top_keywords])}\n\n---\n"
                                ...
                                else: output += "* No explicit action items detected via heuristics.\n"
                                output += "\n### 💡 Key Decisions & Core Insights\n"
                                if extracted_decisions:
                                    for d in extracted_decisions: output += f"* {d.capitalize()}.\n"
                                else: output += "* No explicit corporate decisions flagged.\n"
                                
                                st.session_state.ai_summary = output
                                st.toast("Local TF-IDF Processing Complete!", icon="🧠")
                ...
                if st.session_state.ai_summary:
                    ...
                    st.download_button("📥 Download Analysis Brief", st.session_state.ai_summary, "ScribeAI_Analysis_Brief.md", "text/markdown")
